...
class FileFinder(BaseTool):
    """
    A tool that finds files in the current directory and returns their information.
    """
    name: str = "file_finder"
    description: str = "Finds files in the current directory and returns their information."
    parameters: dict = {
        "type": "object",
        "properties": {
            "blocks": {
                ...
                "items": {
                    "type": "object",
                    "properties": {
                        "name": {"type": "string", "description": "The filename to search for."},
                        "action": {"type": "string", "description": "The action to perform (read, info).", "enum": ["read", "info"]},
                    },
                    "required": ["name"],
                },
                "description": "List of blocks, each containing a filename and action.",
            },
        },
        "required": ["blocks"],
    }
    ...
    def set_workspace(self, workspace_path: str):
        self.work_dir = workspace_path

    def get_parameter_value(
        self, block: Dict[str, Any], param_name: str
    ) -> Optional[Any]:
        """
        Gets a parameter value from a block.

        Args:
            block (Dict[str, Any]): The block containing parameters
            param_name (str): The name of the parameter to get

        Returns:
            Optional[Any]: The parameter value if found, None otherwise
        """
        if not isinstance(block, dict):
            return None
        return block.get(param_name)
