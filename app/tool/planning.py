...
class PlanningTool(BaseTool):
        ...
        "allOf": [
            {
                ...
                "then": {"required": ["command", "plan_id", "title", "steps"]},
            },
            {
                "if": {
                    "properties": {
                        "command": {"enum": ["update", "set_active", "delete"]}
                    },
                    "required": ["command"],
                },
                "then": {"required": ["command", "plan_id"]},
            },
            {
                "if": {
                    "properties": {"command": {"enum": ["mark_step"]}},
                    "required": ["command"],
                },
                "then": {"required": ["command", "step_index"]},
            },
        ],
    }
    ...
    async def execute(
        ...
        **kwargs,
        ...
        # Handle the case where plan_id is not provided for commands that require it
        if command == "create":
            # For create command, plan_id will be generated if not provided
    ...
    def _create_plan(
        ...
        if not plan_id:
            # Generate a unique plan ID if none is provided
            import uuid

            plan_id = f"plan-{uuid.uuid4().hex[:8]}"
            # Log that we're generating a plan_id
            import logging

            logging.getLogger(__name__).info(f"Generated plan_id: {plan_id}")
