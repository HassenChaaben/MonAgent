    ...
    async def create_initial_plan(self, request: str) -> None:
        ...
        for tool_call in response.tool_calls:
            if tool_call.function.name == "planning":
                # Parse the arguments to check if we need to add plan_id
                try:
                    args = json.loads(tool_call.function.arguments or "{}")

                    # If this is a 'create' command and plan_id is missing, add it
                    if args.get("command") == "create":
                        if not args.get("plan_id") and self.active_plan_id:
                            logger.info(
                                f"Adding missing plan_id parameter: {self.active_plan_id}"
                            )
                            args["plan_id"] = self.active_plan_id
                            # Update the tool_call arguments with the modified args
                            tool_call.function.arguments = json.dumps(args)
                        elif not args.get("plan_id") and not self.active_plan_id:
                            # Generate a unique plan ID if none exists
                            import uuid

                            new_plan_id = f"plan-{uuid.uuid4().hex[:8]}"
                            self.active_plan_id = new_plan_id
                            logger.info(
                                f"Generating new plan_id parameter: {new_plan_id}"
                            )
                            args["plan_id"] = new_plan_id
                            # Update the tool_call arguments with the modified args
                            tool_call.function.arguments = json.dumps(args)
        ...
        if not plan_created:
            ...
            # Create a plan with a generated ID
            if not self.active_plan_id:
                import uuid

                self.active_plan_id = f"plan-{uuid.uuid4().hex[:8]}"
                logger.info(f"Generated fallback plan_id: {self.active_plan_id}")
            ...
            try:
                ...
                tool_msg = Message.assistant_message(
                    f"Created default plan with ID: {self.active_plan_id}"
                )
            except Exception as e:
                logger.error(f"Failed to create default plan: {e}")
                tool_msg = Message.assistant_message(f"Error creating plan: {str(e)}")
